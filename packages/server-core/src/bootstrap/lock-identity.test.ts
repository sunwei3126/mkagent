...
describe('parseTasklistImageName', () => {
  it('extracts the image name from a CSV data row', () => {
    expect(parseTasklistImageName('"MkAgent.exe","1234","Console","1","150,000 K"')).toBe('MkAgent.exe')
  })
...
describe('lockHolderMatchesLock (#978)', () => {
  ...
  it('matches on identical execName', () => {
    expect(lockHolderMatchesLock(lockWith('MkAgent.exe'), 'MkAgent.exe', null)).toBe(true)
  })
  ...
  it('rejects a different executable even when its name contains "mkagent"', () => {
    // PID recycled onto e.g. a game process — must NOT keep the brick (#978)
    expect(lockHolderMatchesLock(lockWith('MkAgent.exe'), 'unrelated-mkagent-launcher', null)).toBe(false)
  })

  it('fails open when the live exec name cannot be determined', () => {
    expect(lockHolderMatchesLock(lockWith('MkAgent.exe'), null, null)).toBe(false)
  })

  it('ignores the command line when execName is recorded', () => {
    // cmdline mentions mkagent, but the executable is something else → recycled PID
    expect(lockHolderMatchesLock(lockWith('MkAgent.exe'), 'java', '/usr/bin/java -jar unrelated-mkagent.jar')).toBe(false)
  })

  describe('legacy locks without execName', () => {
    it('falls back to the mkagent-substring heuristic on the command line', () => {
      expect(lockHolderMatchesLock(lockWith(undefined), null, '/Applications/MkAgent.app/Contents/MacOS/MkAgent')).toBe(true)
